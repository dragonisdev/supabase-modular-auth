...
The canonical API specification is [`openapi.yaml`](api/openapi.yaml). It covers all current Express health, authentication, session, OAuth, admin, and Stripe operations. The colocated [`check-openapi.mjs`](api/check-openapi.mjs) script verifies that the generated TypeScript contract is current.
