...
Use `rate-limit-redis` with a selectable command transport: `node-redis` TCP (default) or Upstash REST over HTTPS. Both retain the same atomic Lua scripts, quotas and key namespaces. TCP requires `REDIS_URL` in production and verifies the connection with at most four initial attempts before listening. Its configurable periodic PING and bounded reconnect backoff remain available for long-lived TCP deployments.
...
- `REDIS_URL` and the Upstash REST token are backend-only secrets; external connections use TLS. REST credentials are sent in the Authorization header and redirects are rejected.
